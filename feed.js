...
function main() {
    ...
    Rx.httpGet(hNewsUri).subscribe(function (response) {
        ...
        function refresh() {
            ...
            var cell = Glyffin.colorPath([0, 1]).addNearMajor(1, Glyffin.asciiMultiLine(3, title).pad(metrics.readHeight * 2, metrics.readHeight)).limitHeight(metrics.tapHeight * 2, 0);
            var app = Glyffin.colorPath([0, 0]).addNearMajor(1, cell).addTopMajor(metrics.tapHeight, Glyffin.button());
            presentation = app.present(metrics, glAudience, function () {
                index++;
                refresh();
            });
        }
